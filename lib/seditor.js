...
function getPasteHtml(e) {
    ...
    // img设置最大宽度
    );pasteHtml = pasteHtml.replace(/<img.*?(?:>|\/>)/igm, function (v) {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        ...
                        if (!result.success) {
                            ...
                            // 数据错误
                            _this3._alert('上传图片失败', '上传图片返回结果错误，返回结果 errorCode=' + result.errorCode);
                        } else {
                            if (hooks.customInsert && typeof hooks.customInsert === 'function') {
                                ...
                                // 将图片插入编辑器
                                _this3.insertLinkImg(result.url);
                            }
